	...
	while (i < commands.GetCount()) {
		if (commands[i] == "-cc")
			...
			if (i < commands.GetCount()) {
				...
				Cout() << Compiler::GetName() << " requires that '-bm' parameter be followed by a valid build method" << '\n';
				return false;
			}
		}
			...
			if (i < commands.GetCount()) {
				...
				Cout() << Compiler::GetName() << " requires that '-arch' parameter be followed by a valid build architecture" << '\n';
				return false;
			}
		}
			...
			if (i < commands.GetCount()) {
				...
				Cout() << Compiler::GetName() << " requires that '-class' parameter be followed by a valid class name" << '\n';
				return false;
			}
		}
			...
			i++;
			if (i < commands.GetCount()) {
				Packages.FindAdd(commands[i]);
			}
			else {
				Cout() << Compiler::GetName() << " requires that '-pak' parameter be followed by a valid package path" << '\n';
				return false;
			}
		}
			...
			if (i < commands.GetCount()) {
				...
				Cout() << Compiler::GetName() << " requires that '-file' parameter be followed by a valid file path" << '\n';
				return false;
			}
		}
			...
			if (i + 1 < commands.GetCount()) {
				...
				Cout() << Compiler::GetName() << " requires that '-acp' be followed by two coordinates" << '\n';
				return false;
			}
		}
		else {
